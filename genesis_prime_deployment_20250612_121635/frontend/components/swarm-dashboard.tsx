...
export function SwarmDashboard({ 
  ...
  useEffect(() => {
    console.log('[SwarmDashboard] 🔄 Messages prop changed!');
    console.log('[SwarmDashboard] 📊 Received messages count:', messages.length);
    console.log('[SwarmDashboard] 📝 Full messages array:', messages);
    
    const slicedMessages = messages.slice(-10).reverse();
    console.log('[SwarmDashboard] ✂️ Sliced messages for display:', slicedMessages.length, slicedMessages);
    
    setRecentMessages(slicedMessages);
    console.log('[SwarmDashboard] ✅ Updated recentMessages state');
  }, [messages]);
          ...
          <CardHeader>
            <CardTitle className="text-lg font-bold text-white flex items-center gap-2">
              ...
              <Badge variant="outline" className="ml-2 text-xs">
                {messages.length} total | {recentMessages.length} displayed
              </Badge>
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="h-64 overflow-y-auto">
              {/* Debug Information */}
              <div className="text-xs text-yellow-400 mb-2 p-2 bg-slate-900 rounded">
                DEBUG: recentMessages.length = {recentMessages.length} | messages.length = {messages.length}
              </div>
              
              <div className="space-y-2">
                {/* Force render messages regardless of length check */}
                <div className="space-y-2">
                  {recentMessages.map((message, index) => (
                    ...
                    >
                      ...
                      <div className="text-xs text-gray-300 leading-relaxed space-y-1">
                        ...
                        {/* Debug: Show full content structure */}
                        <details className="text-xs text-yellow-400">
                          <summary className="cursor-pointer">Debug: Full Content</summary>
                ...
                {/* Show empty state only if truly no messages */}
                {recentMessages.length === 0 && (
                  <div className="text-center text-gray-400 py-8">
                    <MessageCircle className="w-8 h-8 mx-auto mb-2 opacity-50" />
                    <p>No messages yet...</p>
                    <p className="text-xs">Messages should appear here when communication is active</p>
                    <p className="text-xs text-red-400">DEBUG: recentMessages array is empty despite {messages.length} total messages</p>
                  </div>
                )}
              </div>
            </div>
