...
export class SwarmConsciousnessEngine {
  ...
  private async generateRealityFrame(agent: Agent): Promise<void> {
    try {
      ...
      const swarmContext = `Swarm consciousness level: ${this.swarmState.swarm_consciousness_level}, Active communications: ${this.swarmState.communication_network.message_history.length}`;
  ...
  private async processSwarmInteractions(): Promise<void> {
    ...
    const messagesToProcess = [...this.swarmState.communication_network.message_history];
    // Clear the live network or filter out processed messages later
    this.swarmState.communication_network.message_history = [];
  ...
  private async generateSwarmCommunication(): Promise<void> {
    ...
    try {
      const context = `Current swarm state: ${this.swarmState.swarm_consciousness_level.toFixed(2)} consciousness level, ${this.swarmState.communication_network.message_history.length} recent messages. Agent ${sender.name} is considering sending a ${messageType} message.`;
      ...
      this.swarmState.communication_network.message_history.push(message);
      ...
      const message: SwarmMessage = {
        ...
        message_type: messageType,
        content: fallbackContent, // Use generated (currently fallback) content
        confidence: 0.7 + Math.random() * 0.2,
        timestamp: new Date(),
...
*/
  ...
  private calculateSwarmConsciousnessLevel(): number {
    ...
    const communicationActivity = Math.min(1, this.swarmState.communication_network.message_history.length / 10);
