...
export function SettingsPanel({ 
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      if (typeof text !== 'string') return toast({ title: 'Upload failed', variant: 'destructive' });
    
      try {
        const uploaded = JSON.parse(text) as ConfigurationState;
    
        if (!uploaded.openRouter || !uploaded.agents) {
          throw new Error('Missing required keys “openRouter” or “agents”.');
        }
    
        setLocalConfig(uploaded);
        setConnectionTestResult(null);
        toast({ title: 'Config loaded', description: 'Review & save.' });
      } catch (err) {
        console.error(err);
        toast({
