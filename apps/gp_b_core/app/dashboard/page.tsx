...
export default function DashboardPage() {
  ...
  const [showGenesisPrimePanel, setShowGenesisPrimePanel] = useState(false);

  // Debug: Track realMessages state changes
  useEffect(() => {
    console.log('[DashboardPage] 🔍 realMessages state changed:', realMessages.length, realMessages);
  }, [realMessages]);
  ...
  const fetchRealMessages = async () => {
    if (!isRealCommunicationEnabled) {
      ...
      toast({ 
        ...
        description: "Real communication is turned off", 
        variant: "default" 
      });
      return;
    }
    ...
    if (isFetchingMessages) {
      console.log('[fetchRealMessages] Already fetching, skipping duplicate request');
      return;
    }
    ...
    try {
      console.log('[fetchRealMessages] 🚀 Starting message fetch from API...');
      toast({ 
        title: "Fetching Messages", 
        description: "Connecting to consciousness backend...", 
        variant: "default" 
      });
      
      const response = await ApiService.getSwarmMessages(15);
      console.log('[fetchRealMessages] ✅ API response received:', response);
      
      const convertedMessages = response.messages.map(convertApiMessageToSwarmMessage);
      console.log('[fetchRealMessages] 🔄 Converted messages:', convertedMessages.length, 'messages');
      
      // Update state with explicit logging
      console.log('[fetchRealMessages] 📝 Updating realMessages state...');
      setRealMessages(convertedMessages);
      
      console.log('[fetchRealMessages] 🌐 Updating swarm state...');
      ...
      console.log('[fetchRealMessages] ✨ Successfully updated all state!');
      toast({ 
        title: "Messages Fetched Successfully", 
        description: `Retrieved ${convertedMessages.length} messages from consciousness backend`, 
        variant: "default" 
      });
      
    } catch (error) {
      console.error('[fetchRealMessages] ❌ Failed to fetch real messages:', error);
      toast({ 
  ...
  useEffect(() => {
    ...
    // Fetch initial real messages after a brief delay to avoid render conflicts
...
const runSwarmTick = () => {
  ...
  <div className="flex items-center space-x-2">
    <Button 
      variant={showGenesisPrimePanel ? "default" : "outline"}
      size="sm"
      onClick={() => setShowGenesisPrimePanel(!showGenesisPrimePanel)}
      className={showGenesisPrimePanel ? "bg-purple-600 hover:bg-purple-700" : "border-slate-600 hover:bg-slate-700 hover:text-slate-50"}
      aria-label="Toggle Genesis Prime Panel"
    >
    ...
    >
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 p-6 overflow-y-auto bg-black/20">
          ...
          {/* Genesis Prime Panel - Conditional Display */}
          {(() => {
            const shouldShow = !isLoading && configuration && swarmState.agents && showGenesisPrimePanel;
            console.log('[DashboardPage] Genesis Prime Panel render check:', {
              isLoading,
              hasConfiguration: !!configuration,
              hasAgents: !!swarmState.agents,
              showGenesisPrimePanel,
              shouldShow
            });
            return shouldShow;
          })() && (
