...
const convertApiMessageToSwarmMessage = (apiMessage: ApiSwarmMessage): SwarmMessage => {
  return {
    ...
    receiver_ids: [], // API doesn't specify receivers, assume broadcast
    ...
    processing_time_ms: 0, // API doesn't provide this
    context_hash: '', // API doesn't provide this
  };
};
...
export default function DashboardPage() {
  ...
  // Fetch messages periodically when simulation is running
  useEffect(() => {
    console.log('[useEffect] Simulation running state changed:', isSwarmRunning);
    if (isSwarmRunning) {
      console.log('[useEffect] Starting message fetch interval (5s)');
      const messageInterval = setInterval(fetchRealMessages, 5000); // Fetch every 5 seconds
      return () => {
